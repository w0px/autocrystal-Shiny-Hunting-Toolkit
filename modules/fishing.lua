...
-- started (battleWatchdog...) or since the last cast/headbutt attempt
-- with no bite/encounter at all (overworldWatchdog...). Being in battle
...
-- a real user report: this used to be sent unconditionally as its own
...
local function do_catch_sequence(isShiny, shinyEmbedFields, shinySpriteUrl)
    ...
    -- Captured early too, same reasoning as the species/name above - this
    -- is what makes it possible to show what item triggered/accompanied
    -- an item-based auto-catch even after the battle (and item_addr's
    -- validity) has ended.
...
-- every single cast, not once per reset.
local FISHING_JITTER_RANGE = 64

local function do_fish_cycle()
    -- Burns a random 1-64 idle frames before every cast so consecutive
    -- bites don't keep landing on the same frame-timing relationship
    -- between the two DV-roll reads. Safe blind here, same as wild.lua -
    -- Gen 2 only rolls an encounter on the bite itself, never while idle
    -- beforehand.
    RngEnabler.enable_randomness(FISHING_JITTER_RANGE)
...
local function register_hooks()
    ...
    Mem.RegisterROMHook(EnemyWildmonInitialized, function()
        ...
        ConsoleLog.log_encounter("fishing", encounterLine)
...
function M.step()
    ...
    -- fishing right now, not just a snapshot from the last encounter.
    --
    ...
    if pendingEncounterUpdate then
        ...
        if stopRequested then
            ...
            -- writeup of the priority bug this fixes (a matching
            -- auto-catch-on-item condition could otherwise return out of
            -- M.step() first, later in this same function, even though a
            -- "stopped" message had already been sent).
            return true
        end
    end
    ...
    if overworld_loaded then
        ...
        elseif os.time() >= overworldWatchdogNextCheckTime then
            ...
            print(string.format("CAST WATCHDOG: no battle started after %d+ seconds of fishing attempts - attempting automatic recovery", stalledFor))
            ...
            if not overworldWatchdogDiscordSent and stalledFor >= BATTLE_WATCHDOG_DISCORD_SECONDS then
                ...
                send_alert(string.format(
                    "\xE2\x9A\xA0\xEF\xB8\x8F Likely stuck while fishing: no battle triggered after over %d seconds, despite automatic recovery attempts. Check on it.",
        ...
        if shinyLatchedThisBattle then
            ...
            if Gui.auto_catch_enabled(hud) then
                ...
                if catchAllowedBySpecies or catchAllowedByItem then
                    ...
                    -- showing the redundant, oddly-ordered third one).
